...
std::unique_ptr<Trajectory> Trajectory::load(SimSetup *sim_setup, const json &j) {
    ...
    if (type == "circular") {
        trajectory = CircularTrajectory::load(sim_setup, params);
    } else if (type == "standstill") {
        trajectory = StandstillTrajectory::load(sim_setup, params);
    } else if (type == "inplace_rotation") {
        trajectory = InplaceRotationTrajectory::load(sim_setup, params);
    } else {
        throw std::runtime_error("Unknown trajectory type.");
    }
    
    trajectory->q_C_B_ = params.at("body_to_camera_rotation").get<tonav::Quaternion>();
