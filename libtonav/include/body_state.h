...
public:
    ...
    static Eigen::Matrix<double, 56, 56> propagateCovariance(
...
private:
    ...
    static Eigen::Matrix<double, 15, 15> bodyStateTransitionMatrix(
        const Filter &filter, const BodyState &from_state,
        BodyState &to_state
    );
    
    static Eigen::Matrix<double, 15, 27> imuCalibrationParamsTransitionMatrix(
        const Filter &filter,
        const BodyState &from_state, BodyState &to_state
    );
    
    static Eigen::Matrix<double, 15, 15> propagationNoiseMatrix(
        const Filter &filter, const BodyState &from_state,
